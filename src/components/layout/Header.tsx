  ...
  return (
    ...
    >
      <div className="container mx-auto flex items-center justify-between">
        ...
        <nav className="hidden md:flex items-center space-x-8">
          {navLinks.map((link) => (
            <Link
              key={link.path}
              to={link.path}
              className="font-medium text-portfolio-light hover:text-portfolio-blue transition-colors"
            >
              {link.title}
            </Link>
          ))}
        ...
        {/* Mobile Menu Button */}
        <button onClick={() => setIsOpen(!isOpen)} className="md:hidden p-2 text-portfolio-light hover:text-portfolio-blue">
          ...
          <nav className="container mx-auto py-4 flex flex-col space-y-4">
            {navLinks.map((link) => (
              <Link
                key={link.path}
                to={link.path}
                className="font-medium text-portfolio-light hover:text-portfolio-blue transition-colors py-2"
